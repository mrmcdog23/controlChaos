...
#import cccore.folder.folder_creator as folder_creator
...
class ShotCreator(base_ui.StandaloneWindowBase):
    ...
    def does_existing_shot_in_list(self, shot_name):
        ...
        """
        existing_shot_names = self.get_text_list_widget_items(self.lw_shot)
    ...
    def get_text_to_item_dict(self, list_widget, new_shot_or_seq=False):
        ...
        """
        item_dict = self.get_text_list_widget_item_dict(list_widget)
        if not new_shot_or_seq:
            return item_dict

        # if only get new filter out to only new items
        new_items_dict = dict()
        for text, item in item_dict.items():
            if item.new_entity:
                new_items_dict[text] = item
    ...
    def enabled_create_button(self):
        ...
        """
        seq_item_dict = self.get_text_to_item_dict(
            self.lw_sequence, new_shot_or_seq=True)
        shot_item_dict = self.get_text_to_item_dict(
            self.lw_shot, new_shot_or_seq=True)
    ...
    def get_entity_list_to_create(self, list_widget):
        ...
        """
        new_items_dict = self.get_text_to_item_dict(
            list_widget, new_shot_or_seq=True)
    ...
    def application_list(self):
        # type: () -> list[str]
        """
        Applications to create on disk

        Returns:
            application_list: List of applications
        """
        application_list = list()
        if self.chk_2dfolders.isChecked():
            application_list.extend(["nuke", "flame"])
        if self.chk_3dfolders.isChecked():
            application_list.extend(["blender", "maya", "houdini"])
    ...
    def create_context(self):
        ...
        """
        if not self.application_list:
            ui_utils.messagebox(
                "No Application", "No applications checked", "critical")
            return

        # build a dictionary of what to create
        create_dict = dict()
        sequences_to_create = self.get_entity_list_to_create(self.lw_sequence)
        shots_to_create = self.get_entity_list_to_create(self.lw_shot)

        # build a dictionary of sequence to shot to create
        for sequence_to_create in sequences_to_create:
            create_dict[sequence_to_create] = list()
        create_dict[self.sequence_name] = shots_to_create

        # add new episodes to dictionary
        create_folders = folder_creator.CreateFolders(
            session=self.ftshot.session, create_dict=create_dict)
        create_folders.create_shots_and_sequences(use_app_list=self.application_list)
