...
class MagicMaskNode(OpenMayaUI.MPxLocatorNode):
    ...
    def initialize(cls):
        ...
        for text_attribute in cls.TEXT_ATTRIBUTES:
            string_data = OpenMaya.MFnStringData().create(text_attribute)
        ...
        cls.middle_text_color = numeric_attr.createColor('middle_text_color', 'middle_text_color')
        numeric_attr.default = (1.0, 0.0, 1.0)
        cls.addAttribute(cls.middle_text_color)
        ...
        cls.top_text_scale = numeric_attr.create(
            'top_text_scale', 'top_text_scale', OpenMaya.MFnNumericData.kFloat, 2.0
        )
...
class MagicMaskDrawOverride(OpenMayaRender.MPxDrawOverride):
    ...
    def prepareForDraw(self, obj_path, camera_path, frame_context, old_data):
        ...
        #
        middle_text_color_r = mask_node.findPlug('middle_text_colorR', False).asFloat()
        middle_text_color_g = mask_node.findPlug('middle_text_colorG', False).asFloat()
        middle_text_color_b = mask_node.findPlug('middle_text_colorB', False).asFloat()
        middle_text_color_a = mask_node.findPlug('bottom_text_alpha', False).asFloat()
        data.middle_text_color = OpenMaya.MColor(
            (middle_text_color_r, middle_text_color_g, middle_text_color_b, middle_text_color_a)
        )
        ...
        cut_frame_enabled = mask_node.findPlug('bottom_center_text', False)
        camera = OpenMaya.MFnCamera(camera_path)
        focal_length_string = 'Focal Length: %.2f' % camera.focalLength
        cut_frame_enabled.setString(focal_length_string)
        return data
    ...
    def addUIDrawables(self, obj_path, draw_manager, frame_context, data):
        ...
        draw_manager.setFontName("Times New Roman")
        ...
        draw_manager.setFontName("Ariel")
        self.draw_text(
        ...
        draw_manager.setFontName("Times New Roman")
        draw_manager.setFontWeight(OpenMayaRender.MUIDrawManager.kWeightBold)
        ...
        draw_manager.setColor(data.middle_text_color)
        draw_manager.setFontSize(int(border_height * 0.25 * data.bottom_text_scale))
        draw_manager.setFontWeight(data.bottom_text_font_weight)
        self.draw_text(
    ...
    def draw_text(draw_manager, position, text, alignment, background_size):
        ...
        draw_manager.text2d(
            ...
            backgroundColor=OpenMaya.MColor((1.0, 0.0, 0.0, 0.0)),
            dynamic=False
        )
